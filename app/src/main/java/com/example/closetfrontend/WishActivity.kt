...
import com.example.closetfrontend.databinding.ActivityWishBinding
import com.example.closetfrontend.databinding.FragmentMyClosetBinding
import retrofit2.Call
import retrofit2.Callback
import retrofit2.Response

class WishActivity : AppCompatActivity(), SwipeRefreshLayout.OnRefreshListener {
    ...
    private lateinit var topAdapter: WishAdapter
    private lateinit var bottomAdapter: WishAdapter
    private lateinit var outerAdapter: WishAdapter
    private lateinit var onepieceAdapter: WishAdapter
    private lateinit var shoesAdapter: WishAdapter
    private lateinit var bagAdapter: WishAdapter
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // 새로고침
        swipeRefreshLayout = binding.wishSwipeLayout
    ...
    private fun initiation() {
        ...
        topAdapter = WishAdapter(topList)
        bottomAdapter = WishAdapter(bottomList)
        outerAdapter = WishAdapter(outerList)
        onepieceAdapter = WishAdapter(onepieceList)
        shoesAdapter = WishAdapter(shoeList)
        bagAdapter = WishAdapter(bagList)
        ...
        for (category in category) {
            api.getWish(userId, category).enqueue(object :
